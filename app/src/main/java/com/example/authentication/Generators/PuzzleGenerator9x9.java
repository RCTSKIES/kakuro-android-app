...
public class PuzzleGenerator9x9 implements PuzzleGenerator {
    ...
    public Grid generateGrid() {
        ...
        Log.d("PuzzleGenerator9x9", "Template: " + Arrays.deepToString(selectedTemplate));
        Log.d("PuzzleGenerator9x9", "Numbers: " + Arrays.deepToString(numbers));
        Log.d("PuzzleGenerator9x9", "Clues: " + Arrays.deepToString(clues));
    ...
    private int[][] generateNumbers(char[][] template) {
        ...
        HashSet<Integer>[] usedNumbersCol = new HashSet[9];
        for (int i = 0; i < 9; i++) {
        ...
        for (int row = 0; row < 9; row++) {
            ...
            for (int col = 0; col < 9; col++) {
                if (template[row][col] == '-') {
                    ...
                    numbers[row][col] = num;
                    Log.e("PuzzleGenerator9x9", "Generated number: " + num);
                }
            }
        }
        return numbers;
    ...
    private int calculateClue(int[][] numbers, char[][] template, int row, int col) {
        ...
        if (col + 1 < 9 && template[row][col + 1] == '-') {
            for (int c = col + 1; c < 9 && template[row][c] == '-'; c++) {
                sum += numbers[row][c];
            }
        }
        // Check if the clue is for a column sum
        else if (row + 1 < 9 && template[row + 1][col] == '-') {
            for (int r = row + 1; r < 9 && template[r][col] == '-'; r++) {
                sum += numbers[r][col];
            }
        }
