...
public class DatabaseManager extends SQLiteOpenHelper {
    ...
    public void saveXP(int levelId, String difficulty, int xp) {
        SQLiteDatabase db = this.getWritableDatabase();
        ContentValues values = new ContentValues();
        values.put(COLUMN_XP, xp);

        db.update(TABLE_LEVELS, values,
                COLUMN_LEVEL_ID + "=? AND " + COLUMN_DIFFICULTY + "=?",
                new String[]{String.valueOf(levelId), difficulty});
        db.close();
    }
    ...
    public int getTotalXP() {
        ...
        // This query treats NULL XP values as 0 when summing
        Cursor cursor = db.rawQuery(
                "SELECT COALESCE(SUM(" + COLUMN_XP + "), 0) FROM " + TABLE_LEVELS,
                null);
    ...
    public void saveLevel(int levelId, String difficulty, Grid grid, GameState.State state, long timeElapsed) {
        SQLiteDatabase db = this.getWritableDatabase();
        ContentValues values = new ContentValues();
